...
In this chapter, we introduce fundamental concepts of the molecular evolution and describe how they are integrated into the field of phylodynamics, a discipline that describes the interaction of evolutionary and ecological dynamics underlying pathogen spread.
Different viral pathogens that emerged in recent history illustrate the vulnerability of the human population to pandemic threats.
...
We subsequently describe some of the most important breakthroughs in the history of molecular evolution, i.e. the introduction of a molecular clock, which allows dating divergence events from genetic data and historical calibration points or sampling times, the shift of the field towards probabilistic methods, the advent of Monte Carlo algorithms as well as powerful and increasingly more attainable computer hardware, which facilitated the adoption of Bayesian approaches.
...
The additional sections of this introductory chapter are devoted towards describing the most basic inference methodology in more details, and constitute the background knowledge for the work presented in Chapters~\ref{chap:epoch}--\ref{chap:pibuss}.
\end{remark}
...
By aiming at the development of a full quantitative understanding of the processes that shape the epidemiology and evolution of RNA viruses, %PL: please add the Grenfell and Holmes 2009 Plos Com Biol reference for this statement
phylodynamics has become a burgeoning area of research.
...
Perhaps the simplest and best-studied model is the ${\mathcal{S} \rightarrow \mathcal{I} \rightarrow \mathcal{R}}$  compartmental model % TODO:c itation
...
Also patterns of geographic diffusion of infectious diseases can be inferred from sequence data \citep{Lemey2009}, sometimes in combination with information on the host movement \citep{Lemey2014}. %PL: add our 2014 GLM-diffusion reference
Integrating population structure into compartmental models which explicitly track the transmission between the infected individuals can provide new avenues for inferring patterns of viral spread. %PL: I think there is a very recent paper by Rasmussen, Volz and Koelle on this

Whereas for emerging pathogens modeling efforts are typically focused on providing estimates of the basic reproduction number, for ongoing epidemics one is typically interested in quantifying the effects of control and preventive measures on between-host scales and the effect of treatment on within-host scale.
...
%PL: got confused an rewrote this.
...
%Evolution is a stochastic process that can introduce at any given moment random base changes in the genetic alphabet.
%For some time those mutations coexist with the vanilla-type %PL: what do you mean with vanilla-type?
%organisms until selection decides whether the mutations increases the fitness, in which case the entire population eventually carries the said mutation and it becomes a fixed substitution.  

Evolution is the product of mutational processes that stochastically introduce changes into the genetic make of organisms and population processes that lead to the elimination, fixation or partial maintenance of these mutations in the population.
...
Molecular phylogenetics studies the evolutionary pathways underlying these processes that lead in the polymorphism data observed in contemporary times.
...
Phylogenetic thinking traces back to the first know figure of a phylogenetic tree, sketched by Charles Darwin in %1937 
1837
on the margin side of his notebook (see Figure \ref{fig:first_trees}), with the famous words \emph{`I think'} written above it.
...
% parsimony
In order to evaluate different tree topologies the idea or parsimony was adopted in phylogenetics. 
Maximum parsimony approaches attempt to find the tree that requires minimum number of evolutionary changes to explain the data. %, maing use of the whole data at once.
This concept stems from the work of William of Ockham (1320), who stated that among competing hypotheses one should select the simplest one and disregard the remaining more complex ones (the Ockham's razor).
%PL: It was really Cavalli-Sforza and Edwards, both students from Fisher, who introduced the idea into phylogenetics. Indeed, Fitch then introduced a tractable algorithm to compute parsimony scores. They also introduced likelihood and in the same way, Felsenstein, then introduced the tractable pruning algorithm to compute likelyhoods. It would be good to find the reference of their paper and add it here as well below in the first sentence on likelihood as both ideas were mentioned in the same paper.
...
\caption{
{ \footnotesize 
{\bf Parsimony example.} The tree on the left is preferred as it requires the least amount changes (3 substitutions as opposed to 4 for the tree on the right).
}% END: footnotesize
}
\label{fig:parsimony}
\end{figure}
...
%likelihood
As a statistical alternative to an optimality criterion, likelihood had also been proposed for phylogenetic inference relatively early.
...
In phylogenetic setting, a likelihood function returns the probability that the proposed model and the proposed tree, understood as a topology and branch lengths, have generated the observed data.
...
The time required to exhaustively search through a space of all possible rooted trees grows explosively with the number of taxa, as depicted in Table \ref{tab:maxLikeBurden}.
...
%bayesian
The advent of powerful and increasingly more attainable compute power has been instrumental to the adoption of statistical methods based on likelihood estimation and the closely related Bayesian methods.
Bayesian statistical thinking is  not a new idea and stems back to the 18th century work of reverend Thomas Bayes, a statistician, philosopher and theologian. 
...
Bayesian statistics have always been appealing as a very flexible approach to inference, as they allow for inclusion of prior belief about any unknown parameters, which, combined with the likelihood of the data, forms an `updated' view of the model, the so-called posterior distribution.
...
Moreover Bayesian inference allows for a natural interpretation of the results in terms of probability distributions. % and not somewhat artificial concepts of p-values or critical 
...
For a long time, this has limited its use for most but the simplest problems until
\cite{Metropolis1953} presented a Metropolis-Hastings method, %PL: Hasting only came in later, around the 70s, if I remember correctly. He essentially generalized the algorithm. So, referring to the 1953 paper for Metropolis-Hastings  seism wrong to me
Markov chain Monte Carlo (MCMC) algorithm for obtaining a sequence of random samples from a probability distribution for which direct sampling is difficult.%\footnote{This algorithm does not have the most peaceful origins, see the remark.} .
...
As opposed to the ultrametric topology assumed for slowly evolving organism (Figure~\ref{fig:ultrametric}), the dates of the samples themselves can be used to calibrate the molecular clock. %PL: please cite Andrew's TipDate paper here
...
\caption{
{ \footnotesize 
%PL: the units of the time scales are hardly visible; please also change 'Heterogenous' to Dated tips and make those labels bigger
{\bf Different tree structures imposed by molecular clock assumptions for organisms evolving at different rates.} The tree on the right represents an evolutionary history of slowly evolving organisms, and has tips sampled in the last decades that are all aligned at $t=0$. Conversely the tree on the left represents an evolutionary history of rapidly evolving organisms with samples at different points in time in the last decades. By enforcing tip heights to be proportional to their sampling dates, temporal information is provided to calibrate the molecular clock for rapidly evolving organisms.
}% END: footnotesize
}
...
Several studies demonstrated that the strict molecular clock hypothesis is often rejected for real-world data sets \citep{EyreWalker1997,Andreasen2001}, leading to widespread evidence for heterogenous rates of evolution.
...
This is due to the fact that genetic distances for branches in the unconstrained model are product of the substitution rate and the branch length in time units.
...
Among others, a random local clock model approach has also been proposed \citet{Drummond2010} as a more discrete alternative, which allows identifying and quantifying a restricted number of rate changes throughout evolutionary history.
...
The process describing how lineages on a genealogy merge to common ancestor as we go back in time from the present is called the \textit{coalescent} \citep{Kingman1982}.
The simplest formulation of the coalescent process described by John Kingman was derived as an approximation of \textit{Wright-Fisher} \citep{Fisher1930,Wright1931} population model (see Figure~\ref{fig:wright-fisher}).
...
In the light of Equation~\ref{eq:exponential_time} we consider the Kingman's coalescent as a continuous time version of the Wright-Fisher model with $n$ sampled individuals.
%Under a molecular clock (see Subsection~\ref{sub:molecular_clock}) mutation rate $r$ and time are confounded.
%%TODO: mutation rate? P(t_k|\theta) ?
...
It is important to note, however, that these models make assumptions that are clearly violated in most populations and therefore lead to population size estimates that generally strongly deviate from census population sizes. 
...
As in changes $N_{e}$ still reflect changes in the census population, the constant population size coalescent model has been extended to account for variable size populations, either in parametric or non-parametric formulations. %PL: add references, e.g. some of the earlier pybus papers fro parametric forms and skyride/skygrid for nonparameterics.
In a Bayesian framework focusing on time-measured trees, such process models find their use as priors on trees.
Coalescent models have also been extended to account for structured populations allowing to estimate migration rates, which naturally brings us to spatial dynamics.  %PL: add references, e.g. Migrate and Lamarc or some of the Hey work for those
...
Like these programs, at the core of BEAST lies a Metropolis-Hasting algorithm (\cite{Metropolis1953, Hasting1970}, see also Subsection~\ref{sub:mcmc}) for constructing Markov chains that converge on the posterior distribution as the equilibrium distribution.
It is precisely why BEAST is not meant for reconstructing a single tree or a point estimate for a parameter of interest, but rather generates samples that can be used to summarize distributions of mutation rates, divergence times, demographic parameters, trait evolution or others.

%beast is cool
When compared to other similar programs, BEAST stands out in the number of models available for performing inference and the flexibility by which they can be combined.
Perhaps the most characteristic point is that BEAST uniquely focuses on sampling rooted, time-measured phylogenies.
This allows for a very natural interpretation of reconstructed trees, and is achieved thanks to the adoption of the molecular clock models, from simple strict clock models, with a uniform rate across the whole time-span of the evolution, to relaxed molecular clock models that model rate variation in various ways \citep{Thorne1998,Yoder2000,Drummond2006,Drummond2010}.
BEAST sports a large number of complementary evolutionary models, which can be combined in a flexible way.
Various substitution models for performing analysis on nucleotide, codon or amino-acid data, accounting for among-site and among-lineage rate variation and partitioned in arbitrary ways, are complemented with demographic tree priors such as coalescent \citep{Drummond2005;Minin2008b} or birth-death priors and trait evolutionary models. %PL: add reference for birth-death
...
%gpus
Growing availability of large complex data-sets has led to the term `Big Data'. %being coined.
Large sample sizes, such as full genome sequences or the ones generated with next-gen sequencing techniques are increasingly challenging phylogenetic inference.
Fortunately strategies have been proposed to reduce the time that it takes to fit complex phylogenetic models to the vast amounts of data.
These solutions are mainly focused on parrallellization and on harnessing the powers of modern graphics processing units (GPUs), such as the NVIDIAs Tesla line of GPUs that target the high performance computing market.
...
% external software
BEAST is packaged with a number of additional utilities that can either assist in generating the input for BEAST, in analyzing the results or in some other more specific tasks.
...
\textbf{LogCombiner} combines log files from multiple runs, that can be than imported into \textbf{Tracer}, a graphical tool for the analysis and visualization of the output of BEAST as well as other common MCMC packages.
...
The software is continuously being improved and new models are being developed, but the development has recently been split into a continuing BEAST1 branch and a new BEAST2 branch. %PL: add BEAST2 reference.
...
Relaxing some of these assumptions and development of more biologically plausible models, able to grasp the complex nature of phylodynamic dispersal processes is an important aim of this thesis \citep{Bielejec2014a}.
...
Part of the research presented in this thesis is devoted to the development of a flexible software for sequence simulation, that integrates both the tree-generative processes, as well as models responsible for shaping the molecular histories and geographical dispersal \citep{Bielejec2014a}.
...
In particular, Bayesian phylogeography  result in posterior distributions of trees, with each tree branch annotated with estimated node locations and other traits of interest.
...
Presenting readily interpretable visual summaries of the inferences in e communicated to the researchers across different fields is one of the more important aspects of this thesis.
...
Specifically, the number of observed differences in the sampled genetic data will be an underestimate the underlying genetic distance (in Figure~\ref{fig:alignment}, three substitutions underlie a single difference in the observed states).
...
%PL: I commented the figure out for now because you are representing coalescent waiting times and not mutational waiting times; just show a number of Poisson mutation trajectories with more mutations overlaid or the idea that we discussed with A,C,G,T states on the Y-axis
...
%%FB: I wanted to show that different paths (diff branch lengths) are independent relaisations, and waiting time is exponential (similar to Marcs 'travelling particle' description from one of your papers). This is in greyscale now, but if you feel lik eit doesn't conect then we can just comment this figure out.
%\begin{figure}[H]
%\centering
...
%PL: Not sure if you want to do a color page print for this figure; it could simply be black/white
...
\caption{
{ \footnotesize 
{\bf  Graphical representation of  a nucleotide substitution model.} A Markov substitution model with 6 parameters describing the symmetric transition rates.
} % END: footnotesize
}
...
%PL: why not use a simpler tree for fewer taxa, not necessarily with dated tips, where you indicate the terms you describe. For example the sample tree for figure 1.9, but without node and branch numbering, and taxon 1, taxon 2,.. as tip labels
...
{\bf Simulating an evolutionary process along a phylogeny.} 
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.6]{likelihood} 
\caption{
{ \footnotesize 
...
% make the labels of the axis bigger, or deleted all together as it is not needed.
...
\noindent
%PL: maybe explicitly state why we move to log likelihoods:
...
However in some cases there may be no suitable objective distribution at hand, in which case one often resorts to the so called \emph{vague} priors having large variances.
...
\caption{
{ \footnotesize 
{\bf MCMC output (trace plot and histogram/density) for estimating the genetic distance under the JC69 model.
...
%PL: this figure is eh... well... not so attractive.. I commented it out in case a better one cannot be found/made
%It is not really needed and not referenced if I am not mistaken
...
%PL: refer to BSSVS which was already mentioned earlier as a way to tackle this.
