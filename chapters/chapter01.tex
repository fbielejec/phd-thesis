...
Also patterns of geographic diffusion of infectious diseases can be inferred from sequence data \citep{Lemey2009}, sometimes in combination with information on the host movement \citep{Lemey2014}. 
...
Phylogenetic thinking traces back to the first know figure of a phylogenetic tree, sketched by Charles Darwin in 1837 on the margin side of his notebook (see Figure \ref{fig:first_trees}), with the famous words \emph{`I think'} written above it.
...
\cite{Metropolis1953} proposed an Markov chain Monte Carlo (MCMC) algorithm for obtaining a sequence of random samples from a probability distribution for which direct sampling is difficult. 
Later \cite{Hasting1970} extended and generalized the algorithm which become to be known as the Metropolis-Hastings method.
...
As opposed to the ultrametric topology assumed for slowly evolving organism (Figure~\ref{fig:ultrametric}), the dates of the samples themselves can be used to calibrate the molecular clock \citep{Rambaut2000}.
...
As in changes $N_{e}$ still reflect changes in the census population, the constant population size coalescent model has been extended to account for variable size populations, either in parametric \citep{Pybus2002} or non-parametric formulations \citep{Minin2008, Gill2013}. 
...
% FB: not sure what you mean, should I add some annotation on that branch? I explicite left it empty, as the rught brancg is longer so it has more substitutions on it than the left one
\begin{figure}[H]
\centering
\begingroup
...
Likelihood is calculated under the binomial model, with probabilities coming from the JC69 nucleotide substitution model. 
...
For reference we now repeat the same procedure, yet this time we set th ewindow size to $w=1.0$.
...
This does not however mean that the algorithm fails, as it will still reach it's stationary distribution \citep{Tierney1994}, yet it needs much more samples to do so than the proposal in Figure~\ref{fig:metropolis}. 
However for a given sample size a more efficient proposal will produce more samples from the posterior distribution.
...
Even for the most basic problems the convergence needs to be formally assesed.
If the iterations of MCMC algorithm, such as M-H, have not proceeded long enough, the collected sample may not be representative of the true posterior (target) distribution.
One recommended approach for assesing convergence of the chain is to compare different simulated sequences starting from different initial values in terms of their within and between chain variance, an approach known as the Gelman and Rubin diagnostics \citep{Cowles1996}. 
If the within chain variation is much less than the between chain variation than the multiple sequences have clearly not converged.
The weigthed average of those variances is then taken as an estimate of the variance of the stationary distribution and is guaranteed to be an unbiased estimator if the starting values come from the stationary distribution.

Trace plots such as the ones in Figures~\ref{fig:metropolis} and~\ref{fig:window_size} are a usefull visual aid for assesing the mixing of a single chain.
...
Convergence assesment procedures can only reliably diagnose situations in which the particular chain has failed to converge in the given simulation time, yet they do not guarantee the converse; i.e. that the chain which seems like a convergent one has in fact explored the whole target distribution.
...
To reduce the impact of those the general practice is to discard the first $10\%$ of the collected samples, often reffered to as the \textit{burn-in}.
...
Although not a problem for convergence, as after reaching the target distribution the samped value sare guaranteed to be \textit{iid}, it poses an efficency problem.
...
One of the techniques for reducing auto correlation is the \textit{thinning}, which skips every $k$-th iteration from the sampled sequence. 
