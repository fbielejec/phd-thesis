...
\begin{remark}{Hidden Markov models\label{ref:infinite_hidden_markov}}
...
Extending the \emph{hidden} $\left\{ C\left(t\right) \right\}$ process to a countable infinite number of hidden states leads to the infinite HMM's.
\end{remark}
...
Such a bias has been attributed to the action of strong purifying selection over long evolutionary time scales, and selection-informed models have been shown to improve branch length estimation, 
% \citep{Wertheim2011}, 
but their implementation in phylogenetic dating software is still lacking.
...
\begin{figure}[h!]
\centering
\includegraphics[scale=0.2]{continuous}
\caption{
{ \footnotesize 
...
The typical bandwidth of a modern card ranges around $300$ GB/s, while a PCI express bus v3.0 has a theoretical peak speed of 15.75 GB/s in each direction.
