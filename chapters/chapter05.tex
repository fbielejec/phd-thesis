...
%TODO: these bits moved to General Intro
% 
...
% There is however no biological motivation to assume a constant pattern of the parameters shaping the substitution process across branches, it is rather a computational convenience.
% In fact the evidence for heterogenous rates of evolution have been found and demonstrated in several studies \citep{EyreWalker1997,Andreasen2001} and the strict molecular clock hypothesis is often rejected for real-world data sets.
...
% 
% Unfortunately allowing for an unconstrained changes in branch substitution rates does not facilitate the main advantage of the molecular clock hypothesis - the ability to date the speciation events on the tree.
...
% %GB: this next sentence is unclear, please rewrite it
% Instead, the RLC model allows for constructing proposal which move both in the tree space and in the clock structure space at the same time, allowing for a joint reconstruction of the topology as well as the divergence time estimation. 
...
% %GB: maybe best to roughly explain BSSVS?

Here we propose an approach similar in spirit to the Random Local Clock model.
%GB: for modelling what concerning the branch rates?
