...
\\
Corrected. There is now a more elaborate description accompanying this figure (see Subsection~\ref{sub:ex_beta_binom}).
...
In response to a comment above, we now provide more detail in Subsection~\ref{sub:phylogeo}.
...
\\
I changed Subsection~\ref{sub:glm_epoch}, explaining that by this sentence we mean the impact of various covariates,  and that by using the MG94 parametrization we do not need to assume altho all rate variation is due variation in non-synonymous rates.
%PL: why not quote it (or at least part of it)?
