...
We also require that the rat ematrix $Q$ characterizing the model is a stochastic matrix (each row sums to 0).
The infinitestimal rate matrix is thus given by:
...
This means that typically any inference performed is focused on inferring the product parameter $\theta=rt$.
...
The time and rate are as we already mentioned typically confounded, therefore will be estimated as a single parameter of interest $\theta$.
The probability that the any descending nucleotide in a sequence is different from the ancestral nucleotide can be obtained by summing over all possible paths, i.e. $p_{ij}(t)=\underset{k\neq i}{\sum}p_{ik}(t)$.

From equation (\ref{eq:jc69Finite}) in subsection \ref{sub:jc69} we know that under the JC69 model those probabilities are equal, therefore the probability that a single site is different between two sequences separated by distance $\theta=rt$ is given by:
...
The red line indicates the maximum likelihood estimate (MLE) $\hat{\theta}$ of parameter $\theta$ which is calculated by setting $\frac{d\ell\left(\mathbf{X}|\theta\right)}{d\theta}=0$, where $l\left(\mathbf{X}|\theta\right)$ is the logarithm of likelihood defined by (\ref{eq:likelihood1}):
...
Phylogeography is a research direction aimed at connecting evolutionary processes, which happen over time, with the processes of spatial distribution, in actual geographical coordinates into a joint spatio-temporal dynamic.
Historically however the phylogeographical inference in molecular epidemiology ignored the reciprocal character of these two processes. 
...
\citet{Lemey2009} first considered the Bayesian reconstruction that allows for joint inference of both the tree and the parameters of the spatial and temporal process that shapes the spread and evolution of e.g. rapidly evolving viruses.
Their approach does not need to fix the tree topology, the branch lengths nor the Markov model parameters, and allows for integration over all possible values requiring only for a prior distribution specification for the parameters that we aim to infer.
...
The process is fully characterized by transition rates $\varphi_{ij}$ between states $i$ and $j$ and they occupy the elements of the transition rate matrix $\phi$.
Unlike before the rate matrix does not necessarily have to be symmetrical, i.e. there exist some $i,j$ such that $\varphi_{ij}\neq\varphi_{ji}$.
Additionaly, a priori one would expect many of the infinitesimal rates to be zero, as unlike the molecular data which typically is dense the location data is sparse, with only one observation per taxon and many connections not observed at all.

The Bayesian framework described in subsections \ref{sub:bayesTheorem} and \ref{sub:MCMC} offers the unique ability to integrate different sources of information about the viruses with their genetic data.
In the project we use this ability to infer the tree topology, the ancestral spatial locations and the character substitution process giving rise to the aligned molecular sequence data 
...
\begin{equation}
P\left(\mathbf{F},Q,\phi|\mathbf{X},\mathbf{Y}\right)\propto 
\underbrace{P(\mathbf{X}|\mathbf{F})}_{\begin{array}{c}
Likelihood
\end{array}}\cdot\underbrace{P(\mathbf{Y}|\mathbf{F})}_{\begin{array}{c}
Likelihood
\end{array}}\cdot 
\underbrace{P(\mathbf{F})}_{\begin{array}{c}
topology\\
prior
\end{array}}\cdot\underbrace{P(Q)}_{\begin{array}{c}
substitution\\
prior
\end{array}}\cdot\underbrace{P(\phi)}_{\begin{array}{c}
substitution\\
prior
\end{array}}
...
The likelihood component for both molecular data $\mathbf{X}$ and location data $\mathbf{Y}$ is calculated using recursive tree pruning as descibed in subsection \ref{sub:likelihood}.
We approximate the joint posterior as written down in (\ref{eq:posteriorPhylogeography}) using MCMC methods we talked about in subsection \ref{sub:MCMC}.
